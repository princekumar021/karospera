...
import { ArrowUpRight, Banknote, CalendarClock, Edit } from "lucide-react";
...
export default function Step6Preview({ goToStep }: Step6Props) {
  ...
  const { fullName, monthlyIncome, goal, goalTargetAmount, recurringExpenses } = values;

  const totalRecurring = recurringExpenses.reduce((sum, exp) => sum + (exp.amount || 0), 0);
  const goalProgress = goalTargetAmount ? (totalRecurring / goalTargetAmount) * 100 : 0;
    ...
    >
      <div className="space-y-4">
        <Card className="bg-card">
          ...
          <CardContent>
             <p className="text-2xl font-bold">${(monthlyIncome - totalRecurring).toLocaleString()}</p>
        ...
        <Card className="bg-card">
          ...
          <CardContent>
            <div className="flex justify-between text-sm mb-1">
              <span>${totalRecurring.toLocaleString()}</span>
              <span className="text-muted-foreground">${(goalTargetAmount || 0).toLocaleString()}</span>
            </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <Card className="bg-card">
            ...
            <CardContent>
               <div className="text-2xl font-bold">{nextBill ? `$${nextBill.amount.toLocaleString()}` : 'N/A'}</div>
